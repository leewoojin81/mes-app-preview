...
import type { PurchaseOrderListResponse, PurchaseOrderRow } from "@/lib/types";
import {
  PURCHASE_ORDER_COLS as DETAIL_COLS,
  PURCHASE_ORDER_SUM_KEYS as SUM_KEYS,
  PURCHASE_ORDER_STATUS_OPTIONS,
} from "@/lib/purchase-order-columns";
...
export default function PurchaseOrdersPage() {
  ...
  const [page, setPage] = useTabState("poPage", 1);
  const [pageSize, setPageSize] = useTabState("poPageSize", 200);

  // 기본 조회 범위: 이번 달 1일 ~ 당일
  const [dateFrom, setDateFrom] = useTabState("poDateFrom", firstDayOfMonth);
  const [dateTo, setDateTo] = useTabState("poDateTo", today);
  const [status, setStatus] = useTabState("poStatus", "");

  const [searchInput, setSearchInput] = useTabState("poSearchInput", "");
  const [search, setSearch] = useTabState("poSearch", "");
  ...
  useEffect(() => {
    ...
    fetch(`/api/purchase-orders?${params.toString()}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data: PurchaseOrderListResponse) => {
        if (cancelled) return;
        setRows(data.rows);
        setTotal(data.total);
  ...
  }, [page, pageSize, dateFrom, dateTo, status, search, refreshKey]);
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-navy">구매발주등록</h1>
          <p className="text-sm text-slate-500 mt-1">
            PUR-01 · ERP &quot;구매발주현황&quot; 발주 리포트 엑셀 업로드(기간별 누적) ·
            거래처·품목별 발주 현황
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              window.location.href = `/api/purchase-orders/export?${buildFilterParams().toString()}`;
            }}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1.5">
          <label className="text-xs font-medium text-slate-500 shrink-0">발주일자</label>
          <DateSegmentInput
            value={dateFrom}
            onChange={(v) => {
              setDateFrom(v);
        ...
        >
          ...
          {PURCHASE_ORDER_STATUS_OPTIONS.map((v) => (
        ...
        <input
          ...
          placeholder="거래처명, 품목코드, 품목정보, 조달처명, 발주번호 검색"
          ...
          <table className="text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={DETAIL_COLS.length + 1} className="text-center py-10 text-slate-400">
                    조건에 맞는 구매발주 데이터가 없습니다. 엑셀 업로드로 시작하세요.
                  </td>
                </tr>
              )}
              {!loading &&
              ...
              <tfoot>
                <tr className="bg-slate-100 font-semibold sticky bottom-0 z-20">
                  ...
                  {DETAIL_COLS.map((col) => {
                    ...
                    const isRowsLabel = col.key === "발주번호";
                    return (
                      <td
                        key={col.key}
...
function UploadModal({
  ...
  const submit = async () => {
    ...
    fd.set("file", file);
    const res = await fetch("/api/purchase-orders/import", { method: "POST", body: fd });
          ...
          {result ? (
            <div className="space-y-4">
              <p className="text-sm text-slate-700">
                ...
                {result.skipped > 0 &&
                  ` (건너뜀(발주일자·발주번호 없음) ${result.skipped.toLocaleString()}건)`}
              </p>
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-500">
                &quot;구매발주현황&quot; 양식(.xlsx) 첫 시트를 읽습니다. 1행은 컬럼 제목이어야
                하며 &quot;발주일자&quot;, &quot;발주번호&quot; 컬럼은 필수입니다. 업로드
                파일의 발주일자 최소~최대 구간을 구해 그 구간의 기존 데이터를 전부 삭제한 뒤
