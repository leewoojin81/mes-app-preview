...
import type {
  DiameterDistributionFiltersResponse,
  DiameterDistributionGroup,
  DiameterDistributionResponse,
} from "@/lib/types";

// 몰드코드 그룹 헤더 셀 배경 — B.C 분포 분석(QC-02)과 같은 연두색을 재사용한다. 몰드코드
// 마다 다른 색을 배정하는 게 아니라 "이건 그룹 헤더다"를 나타내는 구조색이라 전 그룹에
// 같은 색을 쓴다(정체성 인코딩이 아님).
const MOLD_HEADER_BG = "#C6E0B4";
// 히트맵 채움색(파란 계열, 단일 hue) — 값이 클수록 진하게.
const HEAT_RGB = "37, 99, 235";
// 규격 직경(품목직경)이 속한 구간을 강조하는 테두리색.
...
export default function DiameterDistributionPage() {
  const [model, setModel] = useTabState("diaModel", "");
  const [mold, setMold] = useTabState("diaMold", "");
  const [dateFrom, setDateFrom] = useTabState("diaDateFrom", firstDayOfMonth);
  const [dateTo, setDateTo] = useTabState("diaDateTo", today);
  // 형명·몰드 요약(=몰드 전체 가능, work_date를 월 단위로 잘라 행을 만든다 — 작업일자를
  // 한 달 안으로 좁히면 자연히 한 행만 남고, 여러 달을 고르면 자동으로 월별 추이가
  // 되므로 별도 "월별" 토글은 두지 않는다, 2026-08-26 사용자 지적) / HEMA No 상세(=몰드
  // 하나를 골라 HEMA No별로 쪼갬) 2단 토글.
  const [mode, setMode] = useTabState<Mode>("diaMode", "summary");
  const [modelOptions, setModelOptions] = useState<string[]>([]);
  const [moldOptions, setMoldOptions] = useState<string[]>([]);
  const [groups, setGroups] = useState<DiameterDistributionGroup[]>([]);
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/diameter-distribution/filters?${buildParams().toString()}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data: DiameterDistributionFiltersResponse) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    fetch(`/api/diameter-distribution?${params.toString()}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data: DiameterDistributionResponse) => {
        if (cancelled) return;
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      <div>
        <h1 className="text-xl font-bold text-navy">형명별/몰드별 직경 분포 분석</h1>
        <p className="text-sm text-slate-500 mt-1">
          QC-03 · 일일작업현황(PROD-10) 데이터를 직경1/직경2/직경3 샘플 단위로 재집계 ·
      ...
      {blocked ? (
        <p className="text-sm text-slate-400 py-16 text-center bg-white border border-slate-200 rounded-lg">
          {!model ? "형명을 선택하세요." : "몰드코드를 선택하면 HEMA No별 직경 분포가 표시됩니다."}
        </p>
      ) : loading ? (
...
// 형명+몰드코드 조합 하나 = 표 하나(B.C 분포 분석/QC-02와 같은 레이아웃). 몰드코드별로
// 직경 실측값 범위가 다르므로(품목마다 규격이 다름) 구간(bins) 자체가 그룹마다 다르다 —
// 그래서 여러 몰드코드를 한 표에 합치지 않고 몰드코드 그룹마다 표를 따로 그린다.
// 형명/몰드코드 셀은 그 표의 행(HEMA No 상세는 HEMA No별, 요약은 작업월별 한 줄씩) 전체에
// 걸쳐 rowSpan으로 병합한다.
function DiameterHeatmapTable({
  group,
  rowLabel,
}: {
  group: DiameterDistributionGroup;
  rowLabel: string;
}) {
  const { model, mold, specDia, bins, rows } = group;
  const specBinIdx =
    specDia == null ? -1 : bins.findIndex((b) => specDia >= b - 1e-9 && specDia < b + 0.05 - 1e-9);
            ...
            {rows.map((row, li) => (
              <tr key={row.rowKey}>
                ...
                {row.values.map((pct, ci) => {
                  ...
                  // 표시 범위(규격 직경 ± 0.25) 밖 실측값은 맨 앞/뒤 칸에 몰아 넣었으므로,
