...
// 바꿨다 — 몰드코드는 작업일자 기간에만 걸리고(형명 선택과 무관하게 항상 전체 몰드
// 코드가 보여야 형명 없이도 몰드코드부터 바로 고를 수 있다), 형명은 선택된 몰드코드
// (+기간) 안에서 실제 존재하는 형명만 보여준다(이전엔 반대 방향이었음). 몰드코드는
// daily_work_status에 없는 값이라 item_code 단위로 dosu_change_status의 조립투입품목
// (조립투입품목명에 "몰드"가 들어간 행)으로 매핑해 얻는다 — QC-01/QC-02와 같은 방식.
export async function GET(req: NextRequest) {
  ...
  // 전체 범위 기준으로 보여준다.
  const { where: dateOnlyWhere, args: dateOnlyArgs } = buildDiameterDistributionWhere(params, {
  ...
  if (itemCodesForModel.length > 0) {
    ...
    models = modelRows
      .map((r) => r.v)
      // 원본 엑셀에 드물게 섞인 깨진 제어문자 한 글자짜리 값을 걸러내려면 단순 빈 문자열
      // 체크로는 부족하다 — 영문/숫자/한글이 하나라도 있어야 진짜 값으로 본다.
