...
import type { WarehouseTransferListResponse, WarehouseTransferRow } from "@/lib/types";
import {
  WAREHOUSE_TRANSFER_COLS as DETAIL_COLS,
  WAREHOUSE_TRANSFER_SUM_KEYS as SUM_KEYS,
  WAREHOUSE_TRANSFER_REASON_OPTIONS,
  WAREHOUSE_TRANSFER_WAREHOUSE_OPTIONS,
} from "@/lib/warehouse-transfer-columns";
...
export default function WarehouseTransferStatusPage() {
  const [rows, setRows] = useState<WarehouseTransferRow[]>([]);
  ...
  const [fromWarehouse, setFromWarehouse] = useTabState("fromWarehouse", "");
  const [toWarehouse, setToWarehouse] = useTabState("toWarehouse", "");
  const [reason, setReason] = useTabState("reason", "");
  ...
  function buildFilterParams() {
    ...
    if (fromWarehouse) params.set("fromWarehouse", fromWarehouse);
    if (toWarehouse) params.set("toWarehouse", toWarehouse);
  ...
  useEffect(() => {
    ...
    fetch(`/api/warehouse-transfer-status?${params.toString()}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data: WarehouseTransferListResponse) => {
        if (cancelled) return;
        setRows(data.rows);
        setTotal(data.total);
  ...
  }, [page, pageSize, dateFrom, dateTo, fromWarehouse, toWarehouse, reason, search, refreshKey]);
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-navy">창고이동현황</h1>
          <p className="text-sm text-slate-500 mt-1">
            INV-05 · ERP &quot;창고이동현황&quot; 창고간 이동 리포트 엑셀 업로드(기간별 누적)
            · 품목·LOT별 창고간 이동 현황
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              window.location.href = `/api/warehouse-transfer-status/export?${buildFilterParams().toString()}`;
            }}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1.5">
          <label className="text-xs font-medium text-slate-500 shrink-0">이동일자</label>
          <DateSegmentInput
            value={dateFrom}
            onChange={(v) => {
              setDateFrom(v);
        ...
        <select
          ...
          className={`border rounded-md px-2.5 py-2 text-sm bg-white ${
            fromWarehouse ? "border-navy text-navy font-medium" : "border-slate-300 text-slate-600"
          }`}
        >
          <option value="">출고창고 전체</option>
          {WAREHOUSE_TRANSFER_WAREHOUSE_OPTIONS.map((v) => (
        ...
        <select
          ...
          className={`border rounded-md px-2.5 py-2 text-sm bg-white ${
            toWarehouse ? "border-navy text-navy font-medium" : "border-slate-300 text-slate-600"
          }`}
        >
          <option value="">입고창고 전체</option>
          {WAREHOUSE_TRANSFER_WAREHOUSE_OPTIONS.map((v) => (
            <option key={v} value={v}>
              {v}
            </option>
        ...
        <select
          ...
          className={`border rounded-md px-2.5 py-2 text-sm bg-white ${
            reason ? "border-navy text-navy font-medium" : "border-slate-300 text-slate-600"
          }`}
        >
          <option value="">이동사유 전체</option>
          {WAREHOUSE_TRANSFER_REASON_OPTIONS.map((v) => (
            <option key={v} value={v}>
              {v}
            </option>
          ))}
        ...
        <input
          ...
          placeholder="품목코드, 품목명, Lot번호, 이동번호 검색"
          ...
          <table className="text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={DETAIL_COLS.length + 1} className="text-center py-10 text-slate-400">
                    조건에 맞는 창고이동 데이터가 없습니다. 엑셀 업로드로 시작하세요.
                  </td>
                </tr>
              )}
              {!loading &&
              ...
              <tfoot>
                <tr className="bg-slate-100 font-semibold sticky bottom-0 z-20">
                  ...
                  {DETAIL_COLS.map((col) => {
                    ...
                    const isRowsLabel = col.key === "이동번호";
                    return (
                      <td
                        key={col.key}
...
function UploadModal({
  ...
  const submit = async () => {
    ...
    fd.set("file", file);
    const res = await fetch("/api/warehouse-transfer-status/import", { method: "POST", body: fd });
          ...
          {result ? (
            <div className="space-y-4">
              <p className="text-sm text-slate-700">
                ...
                {result.skipped > 0 &&
                  ` (건너뜀(이동일자·이동번호 없음) ${result.skipped.toLocaleString()}건)`}
              </p>
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-500">
                &quot;창고이동현황&quot; 양식(.xlsx) 첫 시트를 읽습니다. 1행은 컬럼 제목이어야
                하며 &quot;이동일자&quot;, &quot;이동번호&quot; 컬럼은 필수입니다. 업로드
                파일의 이동일자 최소~최대 구간을 구해 그 구간의 기존 데이터를 전부 삭제한 뒤
