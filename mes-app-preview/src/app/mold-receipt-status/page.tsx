...
import type { MoldReceiptListResponse, MoldReceiptRow } from "@/lib/types";
import {
  MOLD_RECEIPT_COLS as DETAIL_COLS,
  MOLD_RECEIPT_SUM_KEYS as SUM_KEYS,
  MOLD_RECEIPT_GROUP_OPTIONS,
} from "@/lib/mold-receipt-columns";
...
export default function MoldReceiptStatusPage() {
  ...
  const [group, setGroup] = useTabState("group", "");
  ...
  useEffect(() => {
    ...
    fetch(`/api/mold-receipt-status?${params.toString()}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data: MoldReceiptListResponse) => {
        if (cancelled) return;
        setRows(data.rows);
        setTotal(data.total);
  ...
  }, [page, pageSize, dateFrom, dateTo, group, search, refreshKey]);
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-navy">MOLD입고현황</h1>
          <p className="text-sm text-slate-500 mt-1">
            INV-04 · ERP &quot;MOLD입고현황&quot; 몰드(금형) 입고 리포트 엑셀 업로드(기간별
            누적) · 품목·LOT별 몰드 입고 현황
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              window.location.href = `/api/mold-receipt-status/export?${buildFilterParams().toString()}`;
            }}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <select
          ...
          className={`border rounded-md px-2.5 py-2 text-sm bg-white ${
            group ? "border-navy text-navy font-medium" : "border-slate-300 text-slate-600"
          }`}
        >
          <option value="">품목군 전체</option>
          {MOLD_RECEIPT_GROUP_OPTIONS.map((v) => (
        ...
        <input
          ...
          placeholder="품목코드, 품목정보, 형명, LOT No, 고객사LOT No, 입고전표번호 검색"
          ...
          <table className="text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={DETAIL_COLS.length + 1} className="text-center py-10 text-slate-400">
                    조건에 맞는 MOLD입고 데이터가 없습니다. 엑셀 업로드로 시작하세요.
                  </td>
                </tr>
              )}
              {!loading &&
...
function UploadModal({
  ...
  const submit = async () => {
    ...
    fd.set("file", file);
    const res = await fetch("/api/mold-receipt-status/import", { method: "POST", body: fd });
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md flex flex-col">
        ...
        <div className="px-5 py-4">
          {result ? (
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-500">
                &quot;MOLD입고현황&quot; 양식(.xlsx) 첫 시트를 읽습니다. 1행은 컬럼 제목이어야
