...
import type { ProductShipmentListResponse, ProductShipmentRow } from "@/lib/types";
import {
  PRODUCT_SHIPMENT_COLS as DETAIL_COLS,
  PRODUCT_SHIPMENT_SUM_KEYS as SUM_KEYS,
  PRODUCT_SHIPMENT_SALE_TYPE_OPTIONS,
} from "@/lib/product-shipment-columns";
...
export default function ProductShipmentPage() {
  const [rows, setRows] = useState<ProductShipmentRow[]>([]);
  ...
  const [saleType, setSaleType] = useTabState("saleType", "");
  ...
  useEffect(() => {
    ...
    fetch(`/api/product-shipment?${params.toString()}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data: ProductShipmentListResponse) => {
        if (cancelled) return;
        setRows(data.rows);
        setTotal(data.total);
  ...
  }, [page, pageSize, dateFrom, dateTo, saleType, search, refreshKey]);
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-navy">제품출고등록</h1>
          <p className="text-sm text-slate-500 mt-1">
            SALES-03 · ERP &quot;제품출고현황&quot; 출고 실적 리포트 엑셀 업로드(기간별
            누적) · 거래처·품목별 출고 현황
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              window.location.href = `/api/product-shipment/export?${buildFilterParams().toString()}`;
            }}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1.5">
          <label className="text-xs font-medium text-slate-500 shrink-0">출고일자</label>
          <DateSegmentInput
            value={dateFrom}
            onChange={(v) => {
              setDateFrom(v);
        ...
        <select
          ...
          className={`border rounded-md px-2.5 py-2 text-sm bg-white ${
            saleType ? "border-navy text-navy font-medium" : "border-slate-300 text-slate-600"
          }`}
        >
          <option value="">매출구분 전체</option>
          {PRODUCT_SHIPMENT_SALE_TYPE_OPTIONS.map((v) => (
        ...
        <input
          ...
          placeholder="거래처명, 품목코드, 품목정보, Lotno, 출고번호, PO번호 검색"
          ...
          <table className="text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={DETAIL_COLS.length + 1} className="text-center py-10 text-slate-400">
                    조건에 맞는 제품출고 데이터가 없습니다. 엑셀 업로드로 시작하세요.
                  </td>
                </tr>
              )}
              {!loading &&
              ...
              <tfoot>
                <tr className="bg-slate-100 font-semibold sticky bottom-0 z-20">
                  ...
                  {DETAIL_COLS.map((col) => {
                    ...
                    const isRowsLabel = col.key === "출고번호";
                    return (
                      <td
                        key={col.key}
...
function UploadModal({
  ...
  const submit = async () => {
    ...
    fd.set("file", file);
    const res = await fetch("/api/product-shipment/import", { method: "POST", body: fd });
          ...
          {result ? (
            <div className="space-y-4">
              <p className="text-sm text-slate-700">
                업로드 완료 — 신규 {result.inserted.toLocaleString()}건, 덮어씀{" "}
                {result.updated.toLocaleString()}건
                {result.skipped > 0 &&
                  `, 건너뜀(출고일자·출고번호 없음) ${result.skipped.toLocaleString()}건`}
              </p>
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-500">
                &quot;제품출고현황&quot; 양식(.xlsx) 첫 시트를 읽습니다. 1행은 컬럼 제목이어야
                하며 &quot;출고일자&quot;, &quot;출고번호&quot; 컬럼은 필수입니다. 원본이 커서
                기간을 나눠 여러 번 업로드해도 기존 데이터는 지워지지 않고 누적되며, 같은 출고
                건을 다시 올리면 새로 추가되지 않고 덮어씁니다.
              </p>
