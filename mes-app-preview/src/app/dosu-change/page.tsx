...
import type { DosuChangeListResponse, DosuChangeRow } from "@/lib/types";
import {
  DOSU_CHANGE_COLS as DETAIL_COLS,
  DOSU_CHANGE_SUM_KEYS as SUM_KEYS,
...
export default function DosuChangePage() {
  ...
  // 원본 리포트는 조립 실적 전체를 담고 있고, 이전 지시와 다른 도수로 조립된 행만
  // "이전도수"가 채워진다 — 화면 이름에 맞춰 기본값은 그 행만 보여준다.
  const [changedOnly, setChangedOnly] = useTabState("changedOnly", true);
  ...
  }, [page, pageSize, dateFrom, dateTo, gubun, changedOnly, search, refreshKey]);
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-navy">도수변경등록</h1>
          <p className="text-sm text-slate-500 mt-1">
            PROD-06 · ERP &quot;도수변경등록&quot; 조립 실적 리포트 엑셀 업로드(기간별 누적) ·
            지시 대비 실제 조립도수 변경 현황
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              window.location.href = `/api/dosu-change/export?${buildFilterParams().toString()}`;
            }}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <label
          className={`flex items-center gap-1.5 px-3 py-2 rounded-md text-sm border cursor-pointer select-none ${
            changedOnly
              ? "border-navy text-navy font-medium bg-navy/5"
          ...
          <table className="text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={DETAIL_COLS.length + 1} className="text-center py-10 text-slate-400">
                    조건에 맞는 도수변경등록 데이터가 없습니다. 엑셀 업로드로 시작하세요.
                  </td>
                </tr>
              )}
              {!loading &&
...
function UploadModal({
  ...
  const submit = async () => {
    ...
    fd.set("file", file);
    const res = await fetch("/api/dosu-change/import", { method: "POST", body: fd });
          ...
          {result ? (
            <div className="space-y-4">
              <p className="text-sm text-slate-700">
                ...
                {result.skipped > 0 &&
                  ` (건너뜀(지시일자·검사일자·작업지시번호 없음) ${result.skipped.toLocaleString()}건)`}
              </p>
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-500">
                &quot;도수변경등록&quot; 양식(.xlsx) 첫 시트를 읽습니다. 1행은 컬럼 제목이어야
                하며 &quot;지시일자&quot;, &quot;검사일자&quot;, &quot;작업지시번호&quot; 컬럼은
                필수입니다. 업로드 파일의 검사일자 최소~최대 구간을 구해 그 구간의 기존
