...
import type { InventoryStatusListResponse, InventoryStatusRow } from "@/lib/types";
import {
  INVENTORY_STATUS_COLS as DETAIL_COLS,
  INVENTORY_STATUS_SUM_KEYS as SUM_KEYS,
} from "@/lib/inventory-status-columns";

const PAGE_SIZE_OPTIONS = [50, 100, 200];

// 드롭다운 필터(faceted) — 창고는 실제 컬럼, 나머지는 원본 엑셀 컬럼(detail JSON).
const DROPDOWN_FILTERS = [
  { param: "warehouse", label: "창고" },
...
export default function InventoryStatusPage() {
  ...
  }, [itemGroupInput, itemCodeInput, lotNoInput, setItemGroup, setItemCode, setLotNo, setPage]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/inventory-status/filters?${params.toString()}`, { cache: "no-store" })
      .then((res) => res.json())
  ...
  }, [dropdownFilters, itemGroup, itemCode, lotNo, refreshKey]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/inventory-status?${params.toString()}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data: InventoryStatusListResponse) => {
        if (cancelled) return;
        setRows(data.rows);
        setTotal(data.total);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, pageSize, dropdownFilters, itemGroup, itemCode, lotNo, refreshKey]);
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-navy">현재고현황</h1>
          <p className="text-sm text-slate-500 mt-1">
            INV-02 · ERP &quot;현재고현황&quot; 엑셀 업로드 스냅샷 · 원본 컬럼 전체 표시
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              window.location.href = `/api/inventory-status/export?${buildFilterParams().toString()}`;
            }}
          ...
          <table className="text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={DETAIL_COLS.length + 1} className="text-center py-10 text-slate-400">
                    업로드된 재고 데이터가 없습니다. 엑셀 업로드로 시작하세요.
                  </td>
                </tr>
              )}
              {!loading &&
...
function UploadModal({
  ...
  const submit = async () => {
    ...
    fd.set("file", file);
    const res = await fetch("/api/inventory-status/import", { method: "POST", body: fd });
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md flex flex-col" style={drag.style}>
        ...
        <div className="px-5 py-4">
          {result ? (
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-500">
                &quot;현재고현황&quot; 양식(.xlsx) 첫 시트를 읽습니다. 1행은 컬럼 제목이어야 하며
                &quot;품목코드&quot;·&quot;재고수량&quot; 컬럼은 필수입니다. 업로드하면 기존 재고
                데이터는 전부 지워지고 새 파일 내용으로 교체됩니다.
              </p>
