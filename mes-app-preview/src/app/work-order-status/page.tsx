...
import type { WorkOrderStatusListResponse, WorkOrderStatusRow } from "@/lib/types";
import {
  WORK_ORDER_STATUS_COLS as DETAIL_COLS,
  WORK_ORDER_STATUS_SUM_KEYS as SUM_KEYS,
  WORK_ORDER_STATUS_GUBUN_OPTIONS,
  WORK_ORDER_STATUS_LINE_OPTIONS,
  WORK_ORDER_STATUS_STATUS_OPTIONS,
} from "@/lib/work-order-status-columns";
...
export default function WorkOrderStatusPage() {
  const [rows, setRows] = useState<WorkOrderStatusRow[]>([]);
  ...
  const [line, setLine] = useTabState("line", "");
  const [status, setStatus] = useTabState("status", "");
  ...
  useEffect(() => {
    ...
    fetch(`/api/work-order-status?${params.toString()}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data: WorkOrderStatusListResponse) => {
        if (cancelled) return;
        setRows(data.rows);
        setTotal(data.total);
  ...
  }, [page, pageSize, dateFrom, dateTo, gubun, line, status, search, refreshKey]);
  ...
  return (
    <div className="w-full px-4 sm:px-6 py-6 space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-navy">작업지시현황</h1>
          <p className="text-sm text-slate-500 mt-1">
            PROD-05 · ERP &quot;작업지시현황&quot; 작업지시 실적 리포트 엑셀 업로드(기간별
            누적) · 지시 대비 실적·미생산 현황
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => {
              window.location.href = `/api/work-order-status/export?${buildFilterParams().toString()}`;
            }}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1.5">
          <label className="text-xs font-medium text-slate-500 shrink-0">지시일자</label>
          <DateSegmentInput
            value={dateFrom}
            onChange={(v) => {
              setDateFrom(v);
        ...
        >
          ...
          {WORK_ORDER_STATUS_GUBUN_OPTIONS.map((v) => (
        ...
        <select
          ...
          className={`border rounded-md px-2.5 py-2 text-sm bg-white ${
            line ? "border-navy text-navy font-medium" : "border-slate-300 text-slate-600"
          }`}
        >
          <option value="">라인 전체</option>
          {WORK_ORDER_STATUS_LINE_OPTIONS.map((v) => (
            <option key={v} value={v}>
              {v}
            </option>
          ))}
        ...
        <select
          ...
          className={`border rounded-md px-2.5 py-2 text-sm bg-white ${
            status ? "border-navy text-navy font-medium" : "border-slate-300 text-slate-600"
          }`}
        >
          <option value="">상태 전체</option>
          {WORK_ORDER_STATUS_STATUS_OPTIONS.map((v) => (
            <option key={v} value={v}>
              {v}
            </option>
          ))}
        ...
        <input
          ...
          placeholder="작업지시번호, 품목코드, 품목정보, LOT-NO, 수주번호 검색"
          ...
          <table className="text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={DETAIL_COLS.length + 1} className="text-center py-10 text-slate-400">
                    조건에 맞는 작업지시현황 데이터가 없습니다. 엑셀 업로드로 시작하세요.
                  </td>
                </tr>
              )}
              {!loading &&
              ...
              <tfoot>
                <tr className="bg-slate-100 font-semibold sticky bottom-0 z-20">
                  ...
                  {DETAIL_COLS.map((col) => {
                    ...
                    const isRowsLabel = col.key === "작업지시번호";
                    return (
                      <td
                        key={col.key}
...
function UploadModal({
  ...
  const submit = async () => {
    ...
    fd.set("file", file);
    const res = await fetch("/api/work-order-status/import", { method: "POST", body: fd });
          ...
          {result ? (
            <div className="space-y-4">
              <p className="text-sm text-slate-700">
                ...
                {result.skipped > 0 &&
                  ` (건너뜀(지시일자·작업지시번호 없음) ${result.skipped.toLocaleString()}건)`}
              </p>
            ...
            <div className="space-y-4">
              <p className="text-xs text-slate-500">
                &quot;작업지시현황&quot; 양식(.xlsx) 첫 시트를 읽습니다. 1행은 컬럼 제목이어야
                하며 &quot;지시일자&quot;, &quot;작업지시번호&quot; 컬럼은 필수입니다. 업로드
                파일의 지시일자 최소~최대 구간을 구해 그 구간의 기존 데이터를 전부 삭제한 뒤
