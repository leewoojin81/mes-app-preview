// 일일근태입력(PSN-01) "휴가" 드롭다운 — 출근(선택 안 함)/연차/반차/공가/휴무 순서.
// "결근"을 "휴무"로 이름을 바꿨다(db.ts 마이그레이션이 기존 저장값도 함께 바꿔준다).
export const LEAVE_TYPE_OPTIONS = ["연차", "반차", "공가", "휴무"] as const;

// 휴가구분별 "정상" 기준시간(A안, 2026-09-08 사용자 요청) — 더 이상 그날이 평일/휴일인지는
// 보지 않고 휴가구분만으로 정해진다: 출근 8, 연차 0, 반차 4, 공가 0, 휴무 0. 평일/휴일 판단은
// 생산캘린더(BASE-08)를 기준으로 defaultLeaveTypeForCalendar가 "휴가구분 기본값"을 정할 때만
// 쓰인다.
...
// 제외). 휴가구분별 기준시간(출근 8, 연차 0, 반차 4, 공가 0, 휴무 0)에서 지각·조퇴·외출·
